...
test_that("a PCA is its biplot; profiles = FALSE alone makes it its circle", {
  local_null_device()
  expect_identical(dims_of(ggfacto(fx_pca2())), dims_of(ggpca(fx_pca2())))
  expect_identical(dims_of(ggfacto(fx_pca2(), fx_cars())), dims_of(ggpca(fx_pca2(), fx_cars())))
  expect_identical(dims_of(ggfacto(fx_pca2(), profiles = FALSE)),
                   dims_of(ggpca_cor_circle(fx_pca2())))
  # a supplementary variable keeps the biplot, the individuals left out
  expect_identical(dims_of(ggfacto(fx_pca2(), fx_cars(), sup_vars = cyl, profiles = FALSE)),
                   dims_of(ggpca(fx_pca2(), fx_cars(), sup_vars = cyl, profiles = FALSE)))
  d <- fx_cars()
