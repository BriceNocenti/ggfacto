...
- `model.R` — `mca_model()`, the one reader of an MCA whatever engine made it, `pca_model()`, the units every consumer aggregates over, `axis_coord()` and `is_in_analysis()`.
...
In the course `/home/dev1/github/formations_stat/cours/M2S1/livre/`, we want to teach :
- The new aliases and API for the three analyses.
- The new `interpret()` and `ggfacto()` simplified, readable, user-friendly, straight to the point workflows.
- `hierarchical_clustering()` and `clust_tab()` : look at `dev/hierarchical_clustering.md`
- Sous-populations des ACM avec `filter()`, mais on continuant à passer le data.frame de départ (pour les variables sup sur les graphiques, pour ajouter des variables coordonnées et clusters dans la base de données, etc.) : ne pas trop rentrer dans la technique. Marche avec ACP mais on ne l’apprend que dans ACM:
- We won’t use cor_circle function anymore, `ggfacto(acp)` already give the correlation circle, and can be interactive.
- The course must stop to give many different code, in a confusing way, to teach reading and interpretation : the first analysis of each kind should always have the final and unique and most readable form for it’s workflow, the same that will still have to be copy-pasted and modified in the final exams (not verbose, straight-to-the-point, but enough brief comments to understand). If explanations do different things, they give their own code but after the main one is presented, and overall the more standard the form is the best it is for teaching.
- Only modify the text of the course minimally, so that it matches the new API. When bigger modifications have to be done, or where the way the analysis is teached should be thorougly simplified and rewritten, do not hesitet to signal it in html comments (it it’s easy and straight-to-the-point, do it yourself).
- Look at `formations_stat/CLAUDE.md` to understand the course repo. Look at `formations_stat/.claude/skills/` course conception and course writing skills to know the contraints and usages, but since your aim is not to redo all order and progression and pedagogy, you don’t have to apply and use the strict skill workflow.
- `profiles = TRUE` is now the default, so all explicit calls must be removed, and removing the profiles is now opt-in. Unless exception (to teach something without the noise of the individual points, in very few cases), the profiles in light grey will always be there, which is a reminder of the individuals in the data. (If it’s too much, we may use a lighter gray in the future).
- If, along the way, you think about further simplifications of the workflows and code for the students, do not hesitate to make me propositions.

Also modify `ggfacto` roxygen examples to teach the new workflow : for each of the three main analyses, include the `interpret()` step, but also the `ggfacto()` step, and the `ggfacto(interactive = TRUE)` step (brief straight-to-the point comments only).
