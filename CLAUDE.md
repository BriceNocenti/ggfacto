...
- `mca-data.R` — `multiple_correspondence_analysis()`, fitted on the answer profiles (`MCA2()`, the 0.3.2 fit on the individuals), `ggmca()`, `ggmca_data()` and the MCA's builder.
- `ca.R` — `correspondence_analysis()`, which reads a `tab()` and its supplementary variables, `ca_model()`, and `ggca()`.
...
- `model.R` — `mca_model()`, the one reader of an MCA whatever engine made it, `pca_model()`, the units every consumer aggregates over, `name_axes()`, `axis_coord()` and `is_in_analysis()`.
...
- `ingress.R` — what an analysis remembers of its input: the `<VAR>.NA` levels, the one `excl` rule, the weight rule, the rows it was fitted on (`fitted_rows()`, `align_to_fit()`).
...
- **WARNING** — a graded cell is `<font color><b>`, and a host page's rule on `b` (bootstrap's `strong, b`, on the pkgdown site or in a Quarto theme) beats inheritance and blanks every colour: `tooltip_ink()` gives each widget a rule making a tooltip's bold inherit, `!important` since bootstrap's dark-mode rule is the more specific.
...
The package computes no analysis of its own, the clustering apart (below). `multiple_correspondence_analysis()`, `principal_component_analysis()` and `correspondence_analysis()` are the **ingress normalisers** (`MCA2()` and `PCA2()` are their 0.3.2 names, kept for former code: `PCA2()` an alias, `MCA2()` the former fit on the individuals, below) — tidyselect for `active_vars`/`wt`, a zero weight leaving its row out (FactoMineR crashes on it) and a missing one refused, a missing answer turned into a level `<VAR>.NA`, and one `excl` rule, **exact level names**, `NA` (the default) standing for every missing level, sent to FactoMineR as positions because it renames a level two variables share — while `correspondence_analysis()` reads a `tab()`'s weighted counts, whatever it displays. **A CA's supplementary variables ride its table**, since a CA's data is its table: in a `tab()` of several row or column variables, the first of each is the active table and every other variable gives supplementary rows or columns, passed to FactoMineR as positions; a matrix still takes FactoMineR's own `row.sup` / `col.sup`. An MCA's and a PCA's supplementary variables are added at the graph, from the microdata, as `sup_vars`.

**FactoMineR is fed the answer profiles.** `multiple_correspondence_analysis()` hands `FactoMineR::MCA()` the distinct combinations of active answers, each weighted by the sum of its individuals: identical rows of the indicator table leave X′X unchanged, so the eigenvalues and every result on the levels are the individuals' (to 1e-13, `dev/analysis_engine.md` section 4), 9 times faster at a million rows. The object keeps FactoMineR's class and slots, which the ecosystem reads as usual, with one row of `$ind` per profile. ⚠ So `FactoMineR::HCPC(acm)` would cluster profiles and `print()` counts them: the individuals come back through `axis_coord()` and `hierarchical_clust()`, in `mutate()`. **`MCA2()` keeps 0.3.2's fit on the individuals**, the same ingress with every row its own unit, so that last year's `HCPC(MCA2(...))$data.clust` still lines up with the data; it will be deprecated. `mca_model()` reduces every fit to its profiles, so the two names give the same graphs, tables and clusters.

**One reader, `mca_model()` (`R/model.R`), is the only code that knows how an MCA is stored** — a ggfacto fit, on the profiles or (`MCA2()`) the individuals, a `FactoMineR::MCA()` on individuals, GDAtools' `speMCA()` and `csMCA()` (its subcloud) — and every consumer reads it: the plot model, the tooltips, the clusters, `interpret()`, the teaching plots, the alignment. It holds the profiles and their level codes, each individual's profile (`key`) and weight, the levels by position with their counts, the eigenvalues as one three-column table, and the fitted rows. A supplementary level is the weighted barycentre of its individuals over √λ, summed over the units. ggfacto adds two slots of its own: **`res$axes_names`**, the user's, written by `name_axes()` in the order of the axes (`""` an axis not yet named), read defensively by `theme_facto()` for the axis titles and by `interpret()` for an MCA's or a CA's axis headings (a PCA's axes are `col_var`s, part of the column names, and keep their number); and **`res$source`**, the ingress's, `list(key, w, wt)` — **one integer vector over the data frame the user named**, each row's row of `call$X` (in an MCA fitted on the profiles, its answer profile), `NA` for a row left out, beside each row's weight and the weight column's name. A subset is declared by `filter =`, or read off the call itself: `pc_AGD |> filter(...) |> multiple_correspondence_analysis(...)` is re-run with a hidden row id, and kept only if it gives back exactly the analysed data (a `%>%`, a `select()` or a `slice_sample()` make the subset its own reference); a zero weight leaves its row out too. The fit keeps the order of what it was given — an `arrange()` in the pipe holds — and only the match to the named frame is recorded. Every function that takes the microdata back — `ggfacto()`, `ggmca_3d()`, `clust_tab()`, `hierarchical_clust()`, `axis_coord()` and `is_in_analysis()` inside `mutate()` — goes through `align_to_fit()`, which picks those rows and **re-checks their answers and weights**, so a subset needs no second filter and a reordered data frame is refused rather than misaligned. `correspondence_analysis()` writes the margin names back on `call$X`, which `FactoMineR::CA()` strips, and records in `res$source` each level's variable and name, by position in `call$Xtot`, which `ca_model()` (`R/ca.R`) reads — FactoMineR `make.unique()`s level names. A PCA's graph reads `pca_model()`, its distinct individuals as a cloud of points. **A PCA's missing value sits at its variable's weighted mean**, where it adds nothing to the axes, as an excluded level does in a specific MCA: `principal_component_analysis()` imputes it before FactoMineR (whose own imputation is unweighted, and warns) and records the cells in `res$source$na`; `pca_observed()` puts them back, so the model, the tooltips (`NA` on an individual, a count on an arrow) and `interpret()`'s opening block (observed mean and sd, an `NA` column) describe what was observed. `na = "drop"` leaves those rows out instead, through `fitted_rows()`. FactoMineR renames an MCA's levels in its results (`var_lv` for a level two variables share, `var.y` for a level named y/n) and GDAtools names them `var.lv`, so **their rows are read by position**, through the model alone, never matched by name.
...
- **Soft deprecation, one line, once per session.** Every notice goes through `deprecated_fn()` / `renamed_arg()` (`R/utils.R`): short, naming what to write instead, and pointing to the guide (`GGFACTO_GUIDE`, the site's "Get started" article) for the new usage. Never an error, and never a notice on every call.
...
- **A former name stays a true alias when the old behaviour is the new one** (`PCA2 <- principal_component_analysis`), and a thin wrapper through the same implementation when it is not (`MCA2()`, the former fit on the individuals). New functions get no short name.
...
#### Phase 1r — un seul vecteur d'entiers de la base à l'analyse (DONE)

**`res$source` devient `list(key, w, wt)`, et `key` a une entrée par ligne de la base de référence** — celle que l'utilisateur·rice a nommée : le profil de réponses de la ligne (ACM) ou sa ligne de `call$X` (ACP), `NA` pour une ligne laissée de côté. C'est le vecteur unique que demandait la phase : les lignes de la base et celles de l'analyse se répondent par lui, et `which(!is.na(key))` donne les individus ajustés. `n`, `rows` (qui valait `NULL` pour « toutes », d'où une branche chez chaque consommateur) et **`name` disparaissent** : le nom de la base ne servait qu'à un message d'erreur, qui parle désormais en effectifs (« fitted on 3 246 of the 9 234 rows of its data frame, and `data` has 9 300 »).

**Une seule entrée des lignes, `fitted_rows()`** (`R/ingress.R`), remplace `source_rows()`, `usable_weights()` et la renumérotation des `ind.sup` : la base de référence (le pipe rejoué avec un identifiant caché, comme avant, `reference_rows()`), puis `filter =`, puis les poids nuls, tout en positions de la base. **L'ajustement garde l'ordre voulu par l'utilisateur·rice** : `d |> arrange(age) |> mutate(x = ...) |> MCA2(...)` est ajusté dans l'ordre trié (les profils sont numérotés dans cet ordre, le `$ind` d'une ACP le suit), et seule la correspondance avec `d` est enregistrée. Une ACM en lit ses individus dans l'ordre de la base, par leurs profils ; une ACP retrouve l'ordre de `call$X` en triant les lignes analysées par leur `key`. `ggfacto(res, d, ...)` s'aligne donc sans condition sur ce que contenait le pipe ; c'est `d` qu'on rend ensuite, pas sa version triée, refusée (`test-clust.R` l'épingle). Au passage, un poids manquant **hors** du sous-ensemble n'est plus refusé.

**`filter =`**, dans `multiple_correspondence_analysis()` et `principal_component_analysis()`, juste avant `...` pour qu'aucun appel positionnel n'y arrive : une condition évaluée dans `data`, `NA` valant FALSE comme dans `dplyr::filter()`, refusée en clair si elle n'est pas logique ou ne garde aucune ligne. Même `source`, mêmes valeurs propres et même modèle que le pipe équivalent (épinglé), et il se combine au pipe (intersection), aux poids nuls et à `excl`. Les individus supplémentaires d'une ACP y échappent. ⚠ `tabxplor::tab(filter =)` est marqué *superseded* là-bas ; il se justifie ici parce qu'un sous-ensemble déclaré dans l'appel se prouve toujours, là où un pipe `select()`, `slice_sample()` ou `%>%` ne se prouve pas. `%>%` reste non pris en charge : son expression n'est que `.`, et la reconstruire depuis `sys.calls()` serait fragile ; le refus renvoie à `filter =`.

**Un seul contrôle à chaque alignement, `same_individuals()`** : les réponses actives des lignes retenues doivent redonner le profil de chacune (un `match()` d'entiers par variable contre les codes du modèle, au lieu d'une comparaison de chaînes), et leurs poids, quand `data` a la colonne, ceux de l'ajustement — ce qui attrape deux lignes de même profil et de poids différents échangées. Mesuré : 10 ms contre 19 ms sur 9 234 lignes, 0,26 s contre 1,08 s sur un million de lignes à 10 questions, poids compris. ⚠ Deux lignes échangées aux réponses **et** aux poids identiques passent inaperçues : elles sont indiscernables dans tous les nombres que calcule le paquet. L'ordre-indépendance par contenu (réévaluer le filtre, apparier par profil et poids) a été écartée avec le mainteneur : deux mécanismes, et une quosure stockée qui peut se périmer.

**`is_in_analysis(res)`**, nouvelle exportée : TRUE sur les lignes analysées, FALSE ailleurs (filtrées, de poids nul, individus supplémentaires d'une ACP), dans `filter()` / `mutate()` ou seule sur la base de référence — `pc_AGD |> filter(is_in_analysis(acm)) |> tab(...)` décrit la population analysée sans réécrire la condition. Une AC est refusée en clair : sa population est celle de son tableau, filtrée par `tab()`. `to_data_rows()` prend pour cela un `fill`, et rend toujours un vecteur de la taille de la base hors de `mutate()`.

**Partout où c'est pertinent.** Toutes les fonctions qui reprennent la microdonnée passaient déjà par `fit_rows()` : `ggfacto()`, `ggmca()`, `ggmca_3d()`, `ggpca()`, `clust_tab()`, `hierarchical_clust()`, `axis_coord()`, et désormais `is_in_analysis()` ; les tests les exercent sur un ajustement de sous-ensemble à qui l'on rend la base entière, ACM et ACP. `csMCA()` s'y range naturellement (sa clé est `NA` hors du sous-nuage). Les graphiques pédagogiques et `ggpca_3d()` ne lisent pas la microdonnée.

**`profiles = TRUE` par défaut, partout** (décision du mainteneur, par simplification) : `ggfacto()`, `ggmca()` et `ggmca_data()` dessinent le nuage des individus sans qu'on le demande, comme `ggpca()` le faisait déjà. Pour une ACP, `ggfacto(acp)` s'ouvre donc sur le **biplot** ; le cercle des corrélations seul est `ggfacto(acp, profiles = FALSE)` (ou `ggpca_cor_circle()`). Un appel ancien tourne toujours, mais son graphique a désormais des profils ; `profiles = FALSE` rend l'ancien. `max_profiles` passe de 5 000 à 2 000 (ACM et ACP), pour que ce défaut n'alourdisse pas les widgets.

**Un bogue trouvé** : `clust_tab()` d'une ACP pondérée échouait (`rlang::ensym(wt)`), un symbole passé en valeur par `rlang::exec()` ; injecté (`wt = !!wt`) comme dans les deux autres `tab()` de la fonction.

**Tests** : la matrice des sous-ensembles dans `test-ingress.R` — chaque forme de pipe prouvable, `filter =` seul et combiné, les formes non prouvables (sous-ensemble devenu sa propre référence, base entière refusée en effectifs), et chaque façon dont une base rendue peut différer (triée, raccourcie, allongée, recodée dans ou hors du sous-ensemble, échange de poids, colonne de poids retirée, `mutate()` groupé), plus les ajustements FactoMineR bruts et `csMCA()`. **2 439 assertions** (2 354), 0 échec, 0 avertissement, ~60 s ; `check` 0/0/0. Documentation : en-têtes de `R/ingress.R` et `R/model.R`, `?multiple_correspondence_analysis`, `?principal_component_analysis`, `?is_in_analysis`, `dev/hierarchical_clustering.md` §8.

#### Phase 1s — le cours enseigne un seul geste (DONE)

**Chaque analyse s'écrit désormais d'une seule façon dans `formations_stat`, celle que l'examen fera recopier** : l'analyse, `interpret()`, `ggfacto(…, interactive = TRUE)`, puis `mutate(hierarchical_clust())` et `clust_tab(res, data, clust)`. Livre du M2S1 (03 à 08, annexes), examens 2025 (`Examen`, `Correction`, `Substit`), `agd.md` du skill d'exploration et 13 carnets : plus aucun `ggmca`/`ggca`/`ggpca_cor_circle`/`ggi(gg…())`, `*_interpret()`, `get_footer_tabs()`, `clust_tab(data, …, wt =)`, `fct_recode() |> fct_relevel(sort)`, `profiles = TRUE`, ni hauteur de figure à la main ; `eigenvalues()` montre le tableau des valeurs propres là où le texte l'explique. Le code mort de l'ancienne API commenté (≈ 900 lignes à la fin de `03-ACP`) est retiré, les brouillons en prose restent. Le cercle des corrélations est `ggfacto(acp, profiles = FALSE)` ; le biplot remplace `plot(acp, choix =)` + `gridExtra`. Les classes se nomment par `names =`, et `05-ACM` gagne une courte section « ACM sur une sous-population » (`filter()` dans le pipe, puis la base entière). Carnets reknités : chiffres identiques. Livre rendu en entier, sans erreur.

**Trouvés dans le cours et corrigés** : `04-AC` ne tournait plus (`ca_interpret()` n'est pas exporté) ; `03-ACP` interprétait les axes 1 à 4 de l'ACP à trois variables (l'ACP à sept variables n'était jamais évaluée), et sa correction citait des chiffres qui avaient dérivé ; `04-AC` écrivait les noms d'axes des exercices sur la mauvaise AC ; le graphique `axe-specific` de `08` ne s'affichait pas, et ses pourcentages de variance étaient périmés ; les figures pédagogiques de `05` filtraient la sentinelle disparue `"active_vars"` (réécrites sur `ggmca_data()$vars_data` et `role`) ; la prose de `06` annonçait « trois classes » pour une coupe automatique qui en fait neuf ; `04-AC` disait les votes bleus et les CSP vertes.

**Dans le paquet.** Les exemples des trois analyses enseignent le même geste (`interpret()`, `ggfacto()`, `ggfacto(interactive = TRUE)`) ; celui de l'ACP met `cyl` en supplémentaire au lieu de l'activer. L'arbre de `hierarchical_clust()` se lit dans une figure knitr : la grille de `HCPC()` n'est plus carrée (`respect`), qui la centrait en rétrécissant et coupait sa légende ; chaque texte est réduit à sa cellule ; les barres s'arrêtent à 10 classes (ou `nb_clust`). Les descriptions de `ggpca_cor_circle()` et `ggpca()` disent le défaut de la phase 1r, le biplot. `dev/hierarchical_clustering.md` §10 est le code du cours, et ne range plus `filter =` parmi les idées écartées. **Suite** : 2 439 assertions, 0 échec, 0 avertissement ; `check` 0/0/0.

**Après relecture : `name_axes()` et les valeurs manquantes d'une ACP.**
- **`name_axes(res, "…", "…")`**, exportée : les noms des axes dans leur ordre, `""` pour un axe pas encore nommé (le cours écrit `name_axes(acm, "", "", "")` en attente d'être rempli), `"3" = "…"` pour un axe seul (non enseigné). Graphiques et tableaux d'interprétation (ACM, AC : « Axe 1 : 23,4 % de variance (mod. 83 %) — nom ») les impriment ; une ACP garde ses en-têtes nus, qui sont des noms de colonnes. Écarté : un argument de `ggfacto()`, à répéter sur chaque graphique et à indexer dès `axes = c(2, 1)`. Le cours passe de `acm$axes_names[k] <-` à `name_axes()` ; ses corrections d'exercice font grandir une seule ligne d'un axe à l'autre.
- **Une valeur manquante d'une ACP va à la moyenne pondérée de sa variable**, en silence, là où elle n'ajoute rien aux axes : l'équivalent exact d'`excl` en ACM spécifique. FactoMineR imputait la moyenne NON pondérée (mal centrée sous des poids de sondage) et avertissait. Les cellules sont gardées dans `res$source$na`, et `pca_observed()` les restitue : la bulle d'une flèche dit « missing: 3 (9.4%), placed at the mean », celle d'un individu `mpg: NA`, et le bloc d'ouverture d'`interpret()` donne moyenne et écart-type observés et une colonne `NA`. `na = "drop"` écarte plutôt les lignes. Mesuré sur le bonus de `03-ACP` (1 311 lignes sur 22 655) : deux chiffres de la correction bougent d'un point.
- **Deux bogues latents trouvés** : `weighted.var(na.rm = TRUE)` gardait le poids d'une valeur manquante au dénominateur (la variance d'une imputation à la moyenne) ; `pca_interpret()` appariait les lignes des individus supplémentaires de `call$X` aux poids des seuls actifs.
- **Suite** : 2 470 assertions (+31). Deux messages traduits de plus (56).

**Proposé, pas fait.** (1) `ggpca_cor_circle()` n'est plus enseigné : la garder exportée ou la déprécier. (2) `max_profiles = Inf` à enseigner peut-être (commentaire html dans `05-ACM`).

#### Phase 1t — un guide et un site (DONE)

**Un guide court en deux langues, un site pkgdown dans le thème de tabxplor, un README de quelques lignes.** Le guide (`vignettes/articles/ggfacto.Rmd`, « Get started », la page que vise `GGFACTO_GUIDE`, et `ggfacto-fr.Rmd`, écrit d'abord) montre en ~150 lignes le même geste pour les trois analyses, sur les données des exemples : l'ACP de `mtcars` (moyenne, écart-type et coefficient de variation en tête d'`interpret()`, moyennes au survol, `cyl` en supplémentaire), l'AC `relig × partyid` de `gss_cat` sans non-réponses (13 × 8 modalités, 76 % sur l'axe 1 ; le `tab(color = "contrib")` d'abord, puis `interpret()`, `name_axes()` et les profils au survol), l'ACM de `tea` (taux de Benzécri, profils de réponses et tableau de Burt au survol) et sa classification (l'arbre, six classes nommées, `clust_tab()`, le graphique des classes en statique). Le français reprend le vocabulaire du cours ; l'anglais est écrit pour lui-même, dans le vocabulaire courant de l'AGD. La page d'accueil (`pkgdown/index.Rmd`) : deux paragraphes, le crédit à FactoMineR et à ses quatre auteurs, `interpret(axes = 1:2)` de l'ACM de `tea` et son graphique interactif aux classes colorées. La référence est rangée par usage, chaque sujet une fois : le geste commun aux trois analyses, puis ce qui est propre à chacune, l'interactivité et l'export, le retour aux données, les graphiques pédagogiques, les détails, le déprécié. Le README (texte seul) renvoie au site ; `README.Rmd`, `readme_plot.png` et les neuf `man/figures/README-*.png` disparaissent.

**Pourquoi le guide n'est pas sur le CRAN.** Mesuré : la page autonome du guide pesait 10,3 Mo, dont 8,4 Mo de polices. ggiraph 0.9 embarque en base64 les quatre familles Liberation (sans, serif, mono, symbol) dans toute page autonome, alors que les graphiques ne dessinent qu'en sans. `girafe_widget()` n'embarque plus que Liberation Sans (`gdtools::font_set()`, `ggi(font_set =)` pour une autre) : un widget autonome passe de 9,2 à 3,6 Mo, le guide à 4,75 Mo — encore trop pour une vignette, d'où deux articles web, ni `VignetteBuilder` ni vignette à construire au `check`. Sur le site, les polices sont des fichiers partagés et mis en cache.

**Deux bogues de `ggi(savewidget = TRUE)` trouvés en route.** Le fichier gardait la boîte 960 × 500 de htmlwidgets, qui rogne le graphique dans un cadre : il remplit désormais sa fenêtre ou son `<iframe>`, par `fill_page()` (`R/knit.R`), partagé avec les pages que `ggfacto.widget_dir` écrit. Et `htmlwidgets::saveWidget()` laissait son dossier `<nom>_files` à côté (il le supprime relativement au répertoire courant) : l'enregistrement se fait depuis le dossier cible. Deux tests. Et sur le site, toutes les couleurs des bulles disparaissaient : le `strong, b { color: #000 }` de bootstrap l'emportait sur la couleur héritée du `<font>` ; `tooltip_ink()` y répond dans chaque widget (un test). La page d'accueil dépend de `fill_page()` : pkgdown ne lit qu'un `index.md` déjà tricoté, dont il ne peut pas livrer les scripts d'un widget ; le graphique y est donc une page autonome dans `pkgdown/assets/`, montrée dans un cadre à son propre ratio. Ni `index.md` ni ce fichier ne sont suivis par git : la CI (`pkgdown.yaml`, déploiement depuis main seulement) les tricote avant `build_site_github_pages()`. `dev/site_prune.R` retire aussi `docs/CLAUDE.md`, que pkgdown 2.2 copie pour `llms.txt` (sans effet sur main, qui n'a pas de `CLAUDE.md`).

**Dépendances.** `tabxplor (>= 2.0.1)`, la version du CRAN, au lieu d'un numéro de développement ; `gdtools (>= 0.5.0)` en Imports (déjà installé : ggiraph l'importe), `ggiraph (>= 0.9.6)` (le premier à prendre `font_set`), `Config/Needs/website: pkgdown, BriceNocenti/txtheme`, l'URL du site dans `DESCRIPTION` et `inst/CITATION` ; `LazyData` disparaît (il n'y a pas de `data/`).

**Suite** : 2 498 assertions, 0 échec, 0 avertissement ; `check` 0/0/0. Le site se construit sans problème signalé (`dev/build_site.R`).

**Proposé, pas fait.** (1) `clust_tab()` titre ses colonnes `clust`, `lvs` et `Ensemble` — un mot français dans le guide anglais — au lieu du nom de la variable de classes, de rien, et du `Total` traduit ; `HCPC_tab()` partage ces titres par parité. (2) L'ACM de `tea` est haute (l'axe 2 monte à 2,3 pour `tea shop`) et ses étiquettes se chevauchent au centre : c'est le vrai graphique, sans `ylim`. (3) En anglais, les axes s'intitulent `Axe 1` (le msgid lui-même).


#### Phase 1u — le code du cours 2025 tourne sur la 0.4.0 (DONE)

**Le livre donné aux étudiant·es l'an dernier tourne, bloc par bloc, sur la 0.4.0.** `books/M2_CES_Multi` (`formations_stat@84ab22d`, intact avec ses données dans `~/formations_stat_BACKUP_2026-08-30-1414/`) a été exécuté comme knitr l'exécute — les blocs `eval = FALSE` que les étudiant·es tapaient compris, 232 blocs sur dix chapitres —, une fois sous ggfacto 0.3.2 + tabxplor 1.3.1, une fois sous la 0.4.0 + tabxplor 2.0.1, chaque chapitre dans son propre processus. Rapport et harnais : `~/Documents/ggfacto_backcompat/`. Après correctifs, **aucun appel ggfacto du cours n'échoue**, les valeurs propres sont celles de la 0.3.2 et les **20 variables de classes sont identiques, individu par individu**.

**Ce qui cassait : `FactoMineR::HCPC(MCA2(...))$data.clust` dans `mutate()`** — le cœur de la leçon de CAH et des deux examens 2025 (11 blocs, dont 7 par ricochet : les `cah =` et `HCPC_tab()` sur une colonne jamais créée). `MCA2()` était un alias de l'ajustement sur profils ; `$data.clust` avait 5 312 lignes pour 9 234 individus. **`MCA2()` garde désormais l'ajustement de la 0.3.2 sur les individus**, par la même ingestion (`mca_ingress(units =)`), mêmes arguments et mêmes défauts que le nom long ; il sera déprécié dans quelques versions. `source$key` devient « la ligne de `call$X` de chaque ligne de la base » (la convention de l'ACP), et `mca_model()` réduit **tout** ajustement à ses profils — l'identité sur un ajustement sur profils, +0,8 ms sur pc_AGD —, si bien que les deux noms donnent le même modèle (`X`, `key`, poids, modalités identiques, coordonnées à 1e-14), donc les mêmes graphiques, tableaux et classes, sans une branche de plus chez les consommateurs.

**Les noms courts sont les noms de la 0.3.2, gardés pour l'ancien code** (décision du mainteneur) : `PCA2()` reste un vrai alias, son ancien comportement étant le nouveau (03-ACP : 0 régression) ; `CA2()`, ajouté en 0.4.0 et jamais publié, disparaît ; aucune fonction nouvelle n'a de nom court. **`ggi(iframe =, pixel_width =)`**, arguments de la 0.3.2, sont acceptés avec un avis, puis ignorés (ils échouaient dans ggiraph).

**Écarts gardés, voulus** : un `MCA2()` sans `excl` sur des données manquantes exclut les modalités manquantes (défaut `excl = NA`, choisi par le mainteneur ; le bloc de démonstration de 08-Explo bouge de 0,1 à 0,2 point) ; l'imputation pondérée de `PCA2()` (4e décimale) ; les tableaux d'interprétation refondus ; `HCPC_tab()` identique **cellule pour cellule** (252 pourcentages et effectifs pondérés), sa ligne en double `all_col_vars n` en moins et son `n` imprimé dans `Ensemble` ; les avis de dépréciation, une fois par session.

**Hors de ggfacto, et laissés** : les figures d'enseignant·e de 05-ACM qui lisent les internes de la 0.3.2 (sentinelle `"active_vars"`, `kableExtra::remove_column()` sur `mca_interpret()`), hors périmètre par décision du mainteneur ; trois blocs de 04-AC sur des internes de tabxplor (`:::`). ⚠ **Une régression de tabxplor 2.0.1, API exportée, à corriger là-bas** : `tab_plain(..., num = TRUE, df = TRUE)` rendait un `data.frame` à noms de lignes (1.3.1), il rend un tibble dont la première colonne est la variable en ligne, et `FactoMineR::CA()` s'y arrête (« not quantitative ») — toutes les AC de 04-AC et 08-Explo, 36 blocs. Le harnais remodèle cette sortie pour mener le code jusqu'à `ggca()`, qui passe ensuite partout.
...
#### Phase 1v — publication de la 0.4.0 (DONE)
...
**Publié jusqu'à la soumission.** PR #2 fusionnée par commit de fusion, GitHub Pages sur `gh-pages`, `url_check()` propre, R-CMD-check de `main` et rhub (six plateformes) `Status: OK` sans note, win-builder soumis. La CI a demandé deux correctifs : XQuartz sur le runner macOS (le binaire de gdtools, que ggiraph importe, lie `libXrender`), et un instantané ACP rendu indépendant du gras du `Total` (tabxplor 2.0.1 du CRAN le perd, sa version de développement le garde). win-builder `Status: OK`. **Acceptée par le CRAN le 22 septembre 2026**, sans relecture humaine (contrôles automatiques Debian et Windows sans note) ; tag `v0.4.0` sur le commit de fusion `e67bd06`, release GitHub, `dev` en 0.4.0.9000.
