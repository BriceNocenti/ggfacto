...
**Deux bogues de `ggi(savewidget = TRUE)` trouvés en route.** Le fichier gardait la boîte 960 × 500 de htmlwidgets, qui rogne le graphique dans un cadre : il remplit désormais sa fenêtre ou son `<iframe>`, par `fill_page()` (`R/knit.R`), partagé avec les pages que `ggfacto.widget_dir` écrit. Et `htmlwidgets::saveWidget()` laissait son dossier `<nom>_files` à côté (il le supprime relativement au répertoire courant) : l'enregistrement se fait depuis le dossier cible. Deux tests. La page d'accueil en dépend : pkgdown ne lit qu'un `index.md` déjà tricoté, dont il ne peut pas livrer les scripts d'un widget ; le graphique y est donc une page autonome dans `pkgdown/assets/`, montrée dans un cadre à son propre ratio. Ni `index.md` ni ce fichier ne sont suivis par git : la CI (`pkgdown.yaml`, déploiement depuis main seulement) les tricote avant `build_site_github_pages()`. `dev/site_prune.R` retire aussi `docs/CLAUDE.md`, que pkgdown 2.2 copie pour `llms.txt` — ⚠ tabxplor a le même trou.

**Dépendances.** `gdtools (>= 0.5.0)` en Imports (déjà installé : ggiraph l'importe), `ggiraph (>= 0.9.6)` (le premier à prendre `font_set`), `Config/Needs/website: pkgdown, BriceNocenti/txtheme`, l'URL du site dans `DESCRIPTION` et `inst/CITATION` ; `LazyData` disparaît (il n'y a pas de `data/`).

**Suite** : 2 473 assertions (+3), 0 échec, 0 avertissement ; `check` 0/0/0. Le site se construit sans problème signalé (`dev/build_site.R`).

**Proposé, pas fait.** (1) `clust_tab()` titre ses colonnes `clust`, `lvs` et `Ensemble` — un mot français dans le guide anglais — au lieu du nom de la variable de classes, de rien, et du `Total` traduit ; `HCPC_tab()` partage ces titres par parité. (2) L'ACM de `tea` est haute (l'axe 2 monte à 2,3 pour `tea shop`) et ses étiquettes se chevauchent au centre : c'est le vrai graphique, sans `ylim`. (3) Pour la 0.4.0 : `Imports: tabxplor (>= 2.0.0.9000)` est un numéro de développement, `(>= 2.0.1)` est ce que le CRAN porte. (4) En anglais, les axes s'intitulent `Axe 1` (le msgid lui-même).


#### Phase 1u — back-compat test and fixes

Take the old gitted version of `/home/dev1/github/formations_stat/cours/M2S1/livre/`, the right one before the API changes and that correspond to the code I gave to my students last year, and test if the current dev ggfacto package still work with this old code. Aim : the results the students get can be a bit different but the old code work ; and the new aliases carry the new API and workflows. Make me a report of what breaks, and then the best solution is the one that use the old aliases to carry the old behaviour or close enough using wrappers around the new API.
